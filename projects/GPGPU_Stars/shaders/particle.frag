...
void main() {
    ...
    // Scale by vel
    vec3 pointTempCoord = scaleVel * vec3(pointCoord, 1.0);
    pointCoord = pointTempCoord.xy;

    gl_FragColor = vColor;


    // float f = length( pointCoord ) * flareMult;
    float f = length( pointCoord );
    if ( f > 0.5 ) {
        gl_FragColor.a = 0.0;
        // Drop fragment
        discard;
        ...
        if ( f > 0.4 ) {
            gl_FragColor.a = map(f, 0.4, 0.5, 1.0, 0.0);
        } else {
            gl_FragColor.a = 1.0;
        }
    }
